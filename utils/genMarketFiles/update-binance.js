...
let options = {
	...
	request(options).then(body => {
    	...
    	let assets = lodash.unique(lodash.map(body.data, market => market.baseAsset));
    	let currencies = lodash.unique(lodash.map(body.data, market => market.quoteAsset));
    	let pairs = lodash.map(body.data, market => {
      		return {
