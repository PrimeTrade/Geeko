...
let utils = require(lodashlodashdirname + '/../core/util');

let dirs = utils.dirs();
let CandleBatcher = require(dirs.core + 'candleBatcher');

//some old historical data for geeko 
let candles = [
...
describe('core/candleBatcher', function() {
  	...
  	it('should throw when fed a candle', function() {
    	let candle = lodash.first(candles);
  	...
  	it('should correctly add two candles together', function() {
    	cb = new CandleBatcher(2);
    	let lodashcandles = lodash.first(candles, 2);
    	let first = lodash.first(lodashcandles);
    	let second = lodash.last(lodashcandles);

    	let result = {
      		start: first.start,
      		open: first.open,
      		high: lodash.max([first.high, second.high]),
      		low: lodash.min([first.low, second.low]),
      		close: second.close,
    	...
    	result.vwp = result.vwp / result.volume;

    	let spy = sinon.spy();
    	cb.on('candle', spy);
    	cb.write( lodashcandles );
	
    	let cbResult = lodash.first(lodash.first(spy.args));
